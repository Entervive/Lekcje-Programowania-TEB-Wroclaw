...
<body>
    ...
    <main>
        ...
        <?php
        $polaczenie = mysqli_connect('localhost', 'root', '', 'hodowla');
        $kwerenda = "SELECT DISTINCT s.data_ur, s.miot, r.rasa FROM swinki s JOIN rasy r ON s.rasy_id = r.id WHERE r.id = 1;";
        $zapytanie = mysqli_query($polaczenie, $kwerenda);
        ...
        <?php
        $polaczenie = mysqli_connect('localhost', 'root', '', 'hodowla');
        $kwerenda = "SELECT s.imie, s.cena, s.opis FROM swinki s JOIN rasy r ON s.rasy_id = r.id WHERE r.id = 1;";
        $zapytanie = mysqli_query($polaczenie, $kwerenda);
